...
import com.pia.catalog.sync.model.CatalogConstants;
...
public final class CatalogUtil {
  ...
  public static String stripForPost(Map<String, Object> map) {
    STRIP_POST.forEach(map::remove);
    return JacksonUtil.objectToJson(map);
  }

  public static String stripForPatch(Map<String, Object> map) {
    STRIP_POST.forEach(map::remove);
    STRIP_PATCH.forEach(map::remove);
    return JacksonUtil.objectToJson(map);
  }

  public static String version0(String json) {
    var tree = (ObjectNode) com.pia.commons.util.JacksonUtil.jsonToTree(json);
    tree.put(CatalogConstants.VERSION, "0");
    tree.put("lifecycleStatus", "In design");
    var validFor = com.pia.commons.util.JacksonUtil.getDefaultObjectMapper().createObjectNode();
  ...
  public static String version1(String json) {
    var tree = (ObjectNode) com.pia.commons.util.JacksonUtil.jsonToTree(json);
  ...
  public static boolean requestedEqualsExisting(SingleContext ctx) {
    ...
    return JacksonUtil2.equals(ctx.getId(), json1, json2);
  }
