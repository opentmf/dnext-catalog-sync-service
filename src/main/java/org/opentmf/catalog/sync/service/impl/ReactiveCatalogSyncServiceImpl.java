...
public class ReactiveCatalogSyncServiceImpl implements CatalogSyncService {
  ...
  private Mono<Void> syncProductOfferingPrices(OverallContext context) {
    return sync(catalogSyncProperties.getProductCatalogUrl(), context,
  ...
  private Mono<Void> sync(String baseUrl, OverallContext overall, CatalogType catalogType,
    ...
    return Flux.fromIterable(Arrays.asList(catalogs))
        .concatMap(catalog -> sync1(baseUrl, overall, catalogType, catalog))
        .then();
  }
  ...
  private Function<? super CatalogGetException, ? extends Mono<Void>> handleNotFound(
